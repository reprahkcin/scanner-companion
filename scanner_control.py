    ...
    for profile_path in profile_paths:
        if os.path.exists(profile_path):
            try:
                ...
                print(
                    f"Loaded hardware profile: {profile_name} from {profile_path}")
                return profile
            except Exception as e:
...
HARDWARE_PROFILE = load_hardware_profile(
    SCANNER_CONFIG.get("profile", "raspberry_pi_3axis"))
...
DEFAULT_OUTPUT_DIR = os.path.expanduser(
    SCANNER_CONFIG["capture"]["output_dir"])
...
# Capture presets by object size and quality
# Values map to: perspectives (angles), focus slices per angle, settle delay (seconds)
CAPTURE_PRESETS = {
    "small": {
        "draft": {"perspectives": 72, "focus_slices": 9, "settle_delay": 0.6},
        "good": {"perspectives": 120, "focus_slices": 15, "settle_delay": 0.8},
        "best": {"perspectives": 180, "focus_slices": 25, "settle_delay": 1.0},
    },
    "medium": {
        "draft": {"perspectives": 48, "focus_slices": 5, "settle_delay": 0.5},
        "good": {"perspectives": 72, "focus_slices": 9, "settle_delay": 0.7},
        "best": {"perspectives": 120, "focus_slices": 15, "settle_delay": 0.9},
    },
    "large": {
        "draft": {"perspectives": 36, "focus_slices": 3, "settle_delay": 0.4},
        "good": {"perspectives": 60, "focus_slices": 5, "settle_delay": 0.6},
        "best": {"perspectives": 90, "focus_slices": 9, "settle_delay": 0.8},
    },
}

CAPTURE_PRESET_SIZE_OPTIONS = ["small", "medium", "large"]
CAPTURE_PRESET_QUALITY_OPTIONS = ["draft", "good", "best"]
...
class ScannerGUI(tk.Tk):
    ...
    def __init__(self):
        ...
        try:
            ...
            if "_loaded_from" in SCANNER_CONFIG:
                ...
                self.status_var = tk.StringVar(
                    value=f"Connected to Arduino | Config: {config_file}")
            else:
                self.status_var = tk.StringVar(
                    value="Connected to Arduino | Using default config")
        ...
        self.lens_to_object_mm = tk.DoubleVar(
            value=xmp_config.get("lens_to_object_mm", 250.0))
        self.rail_to_horizon_deg = tk.DoubleVar(
            value=xmp_config.get("rail_to_horizon_deg", 0.0))

        # === Camera calibration parameters for XMP ===
        self.focal_length_35mm = tk.DoubleVar(
            value=xmp_config.get("focal_length_35mm", 50.0))
        self.distortion_model = tk.StringVar(
            value=xmp_config.get("distortion_model", "brown3"))
        self.skew = tk.DoubleVar(value=0.0)  # Camera skew parameter
        self.aspect_ratio = tk.DoubleVar(value=1.0)  # Pixel aspect ratio
        self.principal_point_u = tk.DoubleVar(
            value=0.0)  # Principal point U (normalized)
        self.principal_point_v = tk.DoubleVar(
            value=0.0)  # Principal point V (normalized)
        ...
        # Min tilt angle for spherical scan
        self.elevation_min = tk.DoubleVar(value=-60.0)
        # Max tilt angle for spherical scan
        self.elevation_max = tk.DoubleVar(value=60.0)
        # Number of elevation angles
        self.elevation_steps = tk.IntVar(value=5)
        ...
        self.guided_stack_depth_mm = tk.DoubleVar(
            value=1.0)      # Total stack depth in mm
        self.guided_slices_per_stack = tk.IntVar(
            value=10)        # Number of focus slices per stack
        # Number of vertical tilt levels
        self.guided_tilt_levels = tk.IntVar(value=3)
        self.guided_stacks_per_rotation = tk.IntVar(
            value=36)     # Stacks per full rotation (360°)
        self.guided_start_tilt = tk.DoubleVar(
            value=-30.0)        # Starting tilt angle (bottom)
        self.guided_end_tilt = tk.DoubleVar(
            value=30.0)           # Ending tilt angle (top)
        self.guided_start_rail_mm = tk.DoubleVar(
            value=50.0)      # Near-plane rail position

        # Guided capture state
        # List of (rotation°, tilt°, rail_mm) tuples
        ...
        try:
            ...
            self.rotation_scale.trace_add(
                'write', lambda *_: self._save_app_settings())
            self.rotation_invert.trace_add(
                'write', lambda *_: self._save_app_settings())
        except AttributeError:
            # Older Tk fallback
            self.rotation_scale.trace(
                'w', lambda *_: self._save_app_settings())
            self.rotation_invert.trace(
                'w', lambda *_: self._save_app_settings())
    ...
    def _build_gui(self):
        ...
        self.help_frame = ttk.LabelFrame(
            right_side, text="User Manual", padding=10)
        ...
        self.notes_frame = ttk.LabelFrame(
            right_side, text="Session Notes", padding=10)
    ...
    def _build_manual_tab(self):
        ...
        self.camera_frame = ttk.LabelFrame(
            left_column, text="Camera Preview", padding=10)
        ...
        # --- Motor 1 Frame ---
        self.motor1_frame = ttk.LabelFrame(
            motors_frame, text="Motor 1 (Rotation)", padding=10)
        self.motor1_frame.grid(
            row=0, column=0, padx=(0, 5), pady=5, sticky="nsew")

        self.motor1_pos_var = tk.StringVar(value="0.0°")
        ttk.Label(self.motor1_frame, text="Position:").grid(
            row=0, column=0, sticky="w")
        ttk.Label(self.motor1_frame, textvariable=self.motor1_pos_var, font=(
            "TkDefaultFont", 10, "bold")).grid(row=0, column=1, sticky="w")

        ttk.Label(self.motor1_frame, text="Step Size:").grid(
            row=1, column=0, sticky="w")
        self.motor1_step_var = tk.StringVar(value="1.0")
        self.motor1_step_entry = ttk.Entry(
            self.motor1_frame, textvariable=self.motor1_step_var, width=8)
        self.motor1_step_entry.grid(row=1, column=1, sticky="w")
        ttk.Label(self.motor1_frame, text="degrees").grid(
            row=1, column=2, sticky="w")

        btn_frame1 = ttk.Frame(self.motor1_frame)
        btn_frame1.grid(row=2, column=0, columnspan=3, pady=(10, 0))

        self.btn_m1_ccw = ttk.Button(
            btn_frame1, text="◀ CCW", command=self.motor1_ccw)
        self.btn_m1_ccw.grid(row=0, column=0, padx=(0, 5))

        self.btn_m1_home = ttk.Button(
            btn_frame1, text="⌂ Home", command=self.motor1_home)
        self.btn_m1_home.grid(row=0, column=1, padx=5)

        self.btn_m1_cw = ttk.Button(
            btn_frame1, text="CW ▶", command=self.motor1_cw)
        self.btn_m1_cw.grid(row=0, column=2, padx=(5, 0))

        # --- Motor 2 Frame ---
        self.motor2_frame = ttk.LabelFrame(
            motors_frame, text="Motor 2 (Linear)", padding=10)
        self.motor2_frame.grid(
            row=0, column=1, padx=(5, 0), pady=5, sticky="nsew")

        self.motor2_pos_var = tk.StringVar(value="0.0mm")
        ttk.Label(self.motor2_frame, text="Position:").grid(
            row=0, column=0, sticky="w")
        ttk.Label(self.motor2_frame, textvariable=self.motor2_pos_var, font=(
            "TkDefaultFont", 10, "bold")).grid(row=0, column=1, sticky="w")

        ttk.Label(self.motor2_frame, text="Step Size:").grid(
            row=1, column=0, sticky="w")
        self.motor2_step_var = tk.StringVar(value="1.0")
        self.motor2_step_entry = ttk.Entry(
            self.motor2_frame, textvariable=self.motor2_step_var, width=8)
        self.motor2_step_entry.grid(row=1, column=1, sticky="w")
        ttk.Label(self.motor2_frame, text="mm").grid(
            row=1, column=2, sticky="w")
        ...
        self.btn_m2_home = ttk.Button(
            btn_frame2, text="⌂ Home", command=self.motor2_home)
        ...
        self.motor3_frame = ttk.LabelFrame(
            motors_frame, text="Motor 3 (Tilt)", padding=10)
        self.motor3_frame.grid(
            row=1, column=0, columnspan=2, padx=0, pady=5, sticky="ew")

        self.motor3_pos_var = tk.StringVar(value="0.0°")
        ttk.Label(self.motor3_frame, text="Position:").grid(
            row=0, column=0, sticky="w")
        ttk.Label(self.motor3_frame, textvariable=self.motor3_pos_var, font=(
            "TkDefaultFont", 10, "bold")).grid(row=0, column=1, sticky="w")

        ttk.Label(self.motor3_frame, text="Step Size:").grid(
            row=1, column=0, sticky="w")
        self.motor3_step_var = tk.StringVar(value="1.0")
        self.motor3_step_entry = ttk.Entry(
            self.motor3_frame, textvariable=self.motor3_step_var, width=8)
        self.motor3_step_entry.grid(row=1, column=1, sticky="w")
        ttk.Label(self.motor3_frame, text="degrees").grid(
            row=1, column=2, sticky="w")

        btn_frame3 = ttk.Frame(self.motor3_frame)
        btn_frame3.grid(row=2, column=0, columnspan=3, pady=(10, 0))

        self.btn_m3_down = ttk.Button(
            btn_frame3, text="▼ Down", command=self.motor3_down)
        self.btn_m3_down.grid(row=0, column=0, padx=(0, 5))

        self.btn_m3_home = ttk.Button(
            btn_frame3, text="⌂ Home", command=self.motor3_home)
        self.btn_m3_home.grid(row=0, column=1, padx=5)

        self.btn_m3_up = ttk.Button(
...
• Use consistent lighting during setup"""
        ...
        self.setup_instructions_text.pack(
        ...
        self.setup_instructions_text.config(
            yscrollcommand=instructions_scrollbar.set)
        ...
        ttk.Label(status_info, text="Camera:").grid(
            row=1, column=0, sticky="w")
        ...
        # Motor Power Control
        ttk.Label(status_info, text="Motor Power:").grid(
        ...
        self.btn_motor_power = ttk.Button(
            status_info, text="Turn ON", command=self.toggle_motor_power, width=10)
        ...
        self.rotation_map_frame = ttk.LabelFrame(
            right_column, text="Rotation Calibration", padding=10)
        self.rotation_map_frame.pack(fill="x", pady=(10, 0))

        map_grid = ttk.Frame(self.rotation_map_frame)
        map_grid.pack(fill="x")

        ttk.Label(map_grid, text="Scale Factor:").grid(
            row=0, column=0, sticky="w", pady=2)
        scale_entry = ttk.Entry(
            map_grid, textvariable=self.rotation_scale, width=10)
        ...
        invert_check.grid(row=1, column=0, columnspan=3,
                          sticky="w", pady=(5, 2))
    ...
    def _build_calibration_tab(self):
        ...
        self.cal_camera_frame = ttk.LabelFrame(
            left_column, text="Camera Preview", padding=10)
        ...
        self.cal_motor1_frame = ttk.LabelFrame(
            left_column, text="Rotation Motor", padding=10)
        ...
        step_entry1 = ttk.Entry(
            step_frame1, textvariable=self.cal_motor1_step_var, width=8)
        ...
        ttk.Button(btn_frame1, text="◀ CCW", command=self.cal_motor1_ccw).pack(
            side="left", padx=(0, 5))
        ttk.Button(btn_frame1, text="⌂ Home",
                   command=self.motor1_home).pack(side="left", padx=5)
        ttk.Button(btn_frame1, text="CW ▶", command=self.cal_motor1_cw).pack(
            side="left", padx=(5, 0))

        # Motor 2 controls (linear)
        self.cal_motor2_frame = ttk.LabelFrame(
            left_column, text="Focus Motor (Linear)", padding=10)
        ...
        step_entry2 = ttk.Entry(
            step_frame2, textvariable=self.cal_motor2_step_var, width=8)
        ...
        ttk.Button(btn_frame2, text="▼ Back", command=self.cal_motor2_down).pack(
            side="left", padx=(0, 5))
        ttk.Button(btn_frame2, text="⌂ Home",
                   command=self.motor2_home).pack(side="left", padx=5)
        ttk.Button(btn_frame2, text="▲ Forward", command=self.cal_motor2_up).pack(
            side="left", padx=(5, 0))

        # Motor 3 controls (tilt)
        self.cal_motor3_frame = ttk.LabelFrame(
            left_column, text="Tilt Motor", padding=10)
        ...
        step_entry3 = ttk.Entry(
            step_frame3, textvariable=self.cal_motor3_step_var, width=8)
        ...
        ttk.Button(btn_frame3, text="▼ Down", command=self.cal_motor3_down).pack(
            side="left", padx=(0, 5))
        ttk.Button(btn_frame3, text="⌂ Zero",
                   command=self.motor3_home).pack(side="left", padx=5)
        ttk.Button(btn_frame3, text="▲ Up", command=self.cal_motor3_up).pack(
            side="left", padx=(5, 0))

        # === RIGHT COLUMN: Calibration Workflow ===

        # Calibration status
        self.calibration_status_frame = ttk.LabelFrame(
            right_column, text="Calibration Status", padding=10)
        ...
        self.calibration_step_frame = ttk.LabelFrame(
            right_column, text="Current Step", padding=10)
...
5. Repeat for all cardinal angles"""
        ...
        self.instructions_frame = ttk.LabelFrame(
            right_column, text="Instructions", padding=10)
        ...
        self.calibration_controls_frame = ttk.LabelFrame(
            right_column, text="Calibration Controls", padding=10)
        ...
        self.btn_capture_position.grid(
        ...
        self.btn_load_calibration.grid(
            row=2, column=1, padx=(5, 0), pady=2, sticky="ew")

        # Reset/Start Over button
        self.btn_reset_calibration = ttk.Button(controls_grid, text="Reset / Start Over",
                                                command=self.reset_calibration)
        self.btn_reset_calibration.grid(
            row=3, column=0, columnspan=2, pady=(8, 2), sticky="ew")

        controls_grid.columnconfigure(0, weight=1)
        controls_grid.columnconfigure(1, weight=1)

        # XMP Pose Settings
        self.xmp_settings_frame = ttk.LabelFrame(
            right_column, text="Camera Calibration (XMP)", padding=10)
        ...
        ttk.Label(pose_grid, text="Lens to Object (mm):").grid(
            row=0, column=0, sticky="w", pady=2)
        self.lens_distance_entry = ttk.Entry(
            pose_grid, textvariable=self.lens_to_object_mm, width=12)
        self.lens_distance_entry.grid(
            row=0, column=1, sticky="w", padx=(5, 0), pady=2)

        ttk.Label(pose_grid, text="Focal Length (35mm eq):").grid(
            row=1, column=0, sticky="w", pady=2)
        self.focal_length_entry = ttk.Entry(
            pose_grid, textvariable=self.focal_length_35mm, width=12)
        self.focal_length_entry.grid(
            row=1, column=1, sticky="w", padx=(5, 0), pady=2)

        ttk.Label(pose_grid, text="Position Scale Preset:").grid(
            row=2, column=0, sticky="w", pady=2)
        scale_preset_frame = ttk.Frame(pose_grid)
        scale_preset_frame.grid(
        ...
        ttk.Label(pose_grid, text="Current Scale:").grid(
...
POSITION SCALE PRESET: Keeps XMP precision manageable
  ...
  Note: Scale final model back to real size in RealityCapture"""
        ...
        help_label.grid(row=4, column=0, columnspan=2,
                        sticky="ew", pady=(10, 0))
        ...
        ttk.Label(intrinsics_grid, text="Principal Point U:").grid(
            row=0, column=0, sticky="w", pady=2)
        self.principal_u_entry = ttk.Entry(
            intrinsics_grid, textvariable=self.principal_point_u, width=12)
        self.principal_u_entry.grid(
            row=0, column=1, sticky="w", padx=(5, 0), pady=2)

        ttk.Label(intrinsics_grid, text="Principal Point V:").grid(
            row=1, column=0, sticky="w", pady=2)
        self.principal_v_entry = ttk.Entry(
            intrinsics_grid, textvariable=self.principal_point_v, width=12)
        self.principal_v_entry.grid(
            row=1, column=1, sticky="w", padx=(5, 0), pady=2)

        ttk.Label(intrinsics_grid, text="Aspect Ratio:").grid(
            row=2, column=0, sticky="w", pady=2)
        self.aspect_ratio_entry = ttk.Entry(
            intrinsics_grid, textvariable=self.aspect_ratio, width=12)
        self.aspect_ratio_entry.grid(
            row=2, column=1, sticky="w", padx=(5, 0), pady=2)

        ttk.Label(intrinsics_grid, text="Skew:").grid(
            row=3, column=0, sticky="w", pady=2)
        self.skew_entry = ttk.Entry(
            intrinsics_grid, textvariable=self.skew, width=12)
...
SKEW: Sensor axis alignment
  ...
  - Leave at 0 unless using specialty camera"""
        ...
        help_label2.grid(row=4, column=0, columnspan=2,
                         sticky="ew", pady=(10, 0))
        ...
        ttk.Label(distortion_grid, text="Radial k1:").grid(
            row=0, column=0, sticky="w", pady=2)
        self.k1_entry = ttk.Entry(
            distortion_grid, textvariable=self.distortion_k1, width=12)
        self.k1_entry.grid(row=0, column=1, sticky="w", padx=(5, 0), pady=2)

        ttk.Label(distortion_grid, text="Radial k2:").grid(
            row=1, column=0, sticky="w", pady=2)
        self.k2_entry = ttk.Entry(
            distortion_grid, textvariable=self.distortion_k2, width=12)
        self.k2_entry.grid(row=1, column=1, sticky="w", padx=(5, 0), pady=2)

        ttk.Label(distortion_grid, text="Radial k3:").grid(
            row=2, column=0, sticky="w", pady=2)
        self.k3_entry = ttk.Entry(
            distortion_grid, textvariable=self.distortion_k3, width=12)
        self.k3_entry.grid(row=2, column=1, sticky="w", padx=(5, 0), pady=2)

        ttk.Label(distortion_grid, text="Tangential p1:").grid(
            row=3, column=0, sticky="w", pady=2)
        self.p1_entry = ttk.Entry(
            distortion_grid, textvariable=self.distortion_p1, width=12)
        self.p1_entry.grid(row=3, column=1, sticky="w", padx=(5, 0), pady=2)

        ttk.Label(distortion_grid, text="Tangential p2:").grid(
            row=4, column=0, sticky="w", pady=2)
        self.p2_entry = ttk.Entry(
            distortion_grid, textvariable=self.distortion_p2, width=12)
        self.p2_entry.grid(row=4, column=1, sticky="w", padx=(5, 0), pady=2)

        ttk.Label(distortion_grid, text="Radial k4:").grid(
            row=5, column=0, sticky="w", pady=2)
        self.k4_entry = ttk.Entry(
            distortion_grid, textvariable=self.distortion_k4, width=12)
...
How to calibrate (advanced):
  ...
  4. Enter k1-k4 and p1-p2 values here"""
        ...
        help_label3.grid(row=6, column=0, columnspan=2,
                         sticky="ew", pady=(10, 0))

        # Calibration progress
        self.calibration_progress_frame = ttk.LabelFrame(
            right_column, text="Progress Log", padding=10)
        ...
        self.calibration_progress_text.pack(
    ...
    def _build_capture_and_camera_tab(self):
        ...
        # Specimen settings
        self.specimen_frame = ttk.LabelFrame(
            left_column, text="Specimen", padding=10)

        ttk.Label(self.specimen_frame, text="Name:").grid(
            row=0, column=0, sticky="w")
        self.specimen_entry = ttk.Entry(
            self.specimen_frame, textvariable=self.specimen_name, width=20)
        self.specimen_entry.grid(row=0, column=1, sticky="w", padx=(5, 0))

        ttk.Label(self.specimen_frame, text="Output Dir:").grid(
            row=1, column=0, sticky="w")
        self.output_dir_entry = ttk.Entry(
            self.specimen_frame, textvariable=self.output_dir, width=40)
        self.output_dir_entry.grid(row=1, column=1, sticky="w", padx=(5, 0))

        self.btn_browse_dir = ttk.Button(
            self.specimen_frame, text="Browse", command=self.browse_output_dir)
        self.btn_browse_dir.grid(row=1, column=2, padx=(5, 0))

        # Capture settings
        self.capture_settings_frame = ttk.LabelFrame(
            left_column, text="Capture Settings", padding=10)

        # Scan mode selection
        ttk.Label(self.capture_settings_frame, text="Scan Mode:").grid(
            row=0, column=0, sticky="w")
        scan_mode_frame = ttk.Frame(self.capture_settings_frame)
        scan_mode_frame.grid(row=0, column=1, columnspan=2,
                             sticky="w", padx=(5, 0))
        ...
        # Preset controls
        ttk.Label(self.capture_settings_frame, text="Object Size:").grid(
            row=1, column=0, sticky="w")
        self.preset_size_combo = ttk.Combobox(
            self.capture_settings_frame,
            textvariable=self.capture_preset_size,
            values=CAPTURE_PRESET_SIZE_OPTIONS,
            state="readonly",
            width=10,
        )
        self.preset_size_combo.grid(row=1, column=1, sticky="w", padx=(5, 0))

        ttk.Label(self.capture_settings_frame, text="Quality:").grid(
            row=1, column=2, sticky="w", padx=(10, 0))
        self.preset_quality_combo = ttk.Combobox(
            self.capture_settings_frame,
            textvariable=self.capture_preset_quality,
            values=CAPTURE_PRESET_QUALITY_OPTIONS,
            state="readonly",
            width=10,
        )
        self.preset_quality_combo.grid(
            row=1, column=3, sticky="w", padx=(5, 0))

        self.btn_apply_preset = ttk.Button(
            self.capture_settings_frame,
            text="Apply Preset",
            command=self.apply_capture_preset,
        )
        self.btn_apply_preset.grid(
            row=2, column=1, sticky="w", padx=(5, 0), pady=(4, 0))

        ttk.Label(self.capture_settings_frame, textvariable=self.capture_preset_applied_var,
                  foreground="#666").grid(row=2, column=2, columnspan=2, sticky="w", padx=(10, 0), pady=(4, 0))

        ttk.Label(self.capture_settings_frame, text="Perspectives (angles):").grid(
            row=3, column=0, sticky="w")
        self.stacks_spinbox = ttk.Spinbox(self.capture_settings_frame, from_=1, to=360,
                                          textvariable=self.stacks_count, width=10)
        self.stacks_spinbox.grid(row=3, column=1, sticky="w", padx=(5, 0))

        ttk.Label(self.capture_settings_frame, text="Focus slices per angle:").grid(
            row=4, column=0, sticky="w")
        self.shots_spinbox = ttk.Spinbox(self.capture_settings_frame, from_=1, to=360,
                                         textvariable=self.shots_per_stack, width=10)
        self.shots_spinbox.grid(row=4, column=1, sticky="w", padx=(5, 0))

        # Show computed angle step to clarify spacing
        self.angle_step_label_var = tk.StringVar(value="Angle step: —")
        ttk.Label(self.capture_settings_frame, textvariable=self.angle_step_label_var).grid(
            row=3, column=2, columnspan=2, sticky="w", padx=(10, 0))

        # Spherical scan settings (initially hidden)
        self.spherical_settings_label = ttk.Label(
            self.capture_settings_frame, text="Elevation angles:")
        self.spherical_min_label = ttk.Label(
            self.capture_settings_frame, text="Min:")
        self.spherical_min_spinbox = ttk.Spinbox(self.capture_settings_frame, from_=-60, to=60,
                                                 textvariable=self.elevation_min, width=8)
        self.spherical_max_label = ttk.Label(
            self.capture_settings_frame, text="Max:")
        self.spherical_max_spinbox = ttk.Spinbox(self.capture_settings_frame, from_=-60, to=60,
                                                 textvariable=self.elevation_max, width=8)
        self.spherical_steps_label = ttk.Label(
            self.capture_settings_frame, text="Steps:")
        ...
        ttk.Label(self.capture_settings_frame, text="Settle Delay (s):").grid(
        ...
        self.settle_spinbox.grid(row=6, column=1, sticky="w", padx=(5, 0))

        ttk.Label(self.capture_settings_frame, text="Image Format:").grid(
        ...
        try:
            self.stacks_count.trace_add(
                'write', lambda *args: _update_angle_step_label())
        except AttributeError:
            # Fallback for older Tk versions
            self.stacks_count.trace(
                'w', lambda *args: _update_angle_step_label())
        _update_angle_step_label()

        # Capture controls
        self.capture_controls_frame = ttk.LabelFrame(
            left_column, text="Controls", padding=10)
        ...
        self.progress_frame = ttk.LabelFrame(
            left_column, text="Progress", padding=10)
        ...
        ttk.Label(self.progress_frame, textvariable=self.progress_label_var).grid(
            row=1, column=0, sticky="w")

        # Capture log
        self.capture_log_frame = ttk.LabelFrame(
            left_column, text="Capture Log", padding=10)

        self.capture_log_text = tk.Text(
            self.capture_log_frame, height=8, width=60)
        ...
        # Camera preview
        self.camera_preview_frame = ttk.LabelFrame(
            right_column, text="Camera Preview", padding=10)
        self.camera_preview_label = tk.Label(
            self.camera_preview_frame, bg="black", anchor="center")
        ...
        # Camera controls
        self.camera_controls_frame = ttk.LabelFrame(
            right_column, text="Camera Controls", padding=10)
        ...
        ttk.Label(left_col, text="Shutter Speed (μs):").grid(
        ...
        ttk.Label(left_col, text="(0 = auto)", foreground="#666", font=(
            "TkDefaultFont", 8)).grid(row=2, column=0, sticky="w", pady=(0, 10))

        ttk.Checkbutton(left_col, text="Auto Exposure", variable=self.ae_enable).grid(
            row=3, column=0, sticky="w", pady=(0, 10))

        ttk.Label(left_col, text="ISO:").grid(
        ...
        ttk.Label(left_col, text="Analogue Gain:").grid(
        ...
        ttk.Label(left_col, text="EV Compensation:").grid(
        ...
        ttk.Label(left_col, text="Brightness:").grid(
        ...
        ttk.Label(middle_col, text="WB Red Gain:").grid(
        ...
        ttk.Label(middle_col, text="WB Blue Gain:").grid(
        ...
        ttk.Label(right_col, text="Saturation:").grid(
        ...
        ttk.Label(right_col, text="Sharpness:").grid(
            row=4, column=0, sticky="w", pady=(0, 5))
        ...
        # Resolution controls
        self.resolution_frame = ttk.LabelFrame(
            right_column, text="Resolution", padding=10)
        ttk.Label(self.resolution_frame, text="Preview Resolution:").grid(
            row=0, column=0, sticky="w")
        ...
        ttk.Label(self.resolution_frame, text="Capture Resolution:").grid(
            row=1, column=0, sticky="w")
        ...
        # Camera info
        self.camera_info_frame = ttk.LabelFrame(
            right_column, text="Camera Information", padding=10)
        self.camera_info_text = tk.Text(
            self.camera_info_frame, height=10, width=60, state="disabled")
        self.camera_info_text.grid(row=0, column=0, sticky="nsew")
        info_scrollbar = ttk.Scrollbar(
            self.camera_info_frame, orient="vertical", command=self.camera_info_text.yview)
    ...
    def _layout_gui(self):
        ...
        self.status_bar.pack(side="left", fill="x",
                             expand=True, in_=self.status_frame)
        ...
        self.topmost_button.pack(
            side="right", padx=(5, 0), in_=self.status_frame)
    ...
    def send_motor_command(self, command, wait_for_done=False, timeout=10):
        ...
        try:
            ...
            # Timeout
            self.status_var.set(
                f"Timeout waiting for response to '{command}'. Last: '{last_seen}'")
            return False
        except Exception as e:
...
# =====================================================================
    ...
    def start_calibration(self):
        ...
        self.log_calibration(
            "Calibration started. Position specimen at 0° and set near focus position.")
    ...
    def capture_calibration_position(self):
        ...
        self.log_calibration(
            f"Captured {focus_type} position at {angle}°: {position:.2f}mm")
        ...
        if focus_type == "near":
            ...
            self.log_calibration(
                "Now set the far focus position and capture again.")
        else:
    ...
    def next_calibration_step(self):
        ...
        if (current_angle in self.calibration_data and
            ...
            self.log_calibration(
                f"Moving to {next_angle}°. Set near focus position.")
            self.move_to_angle_async(next_angle)
        else:
            messagebox.showwarning(
                "Calibration", "Please capture both near and far positions for current angle.")
    ...
    def complete_calibration(self):
        ...
        try:
            ...
            if lens_dist <= 0:
                messagebox.showwarning(
                    "XMP Settings", "Please enter a valid lens to object distance (> 0)")
                return

        except ValueError:
            messagebox.showwarning(
                "XMP Settings", "Please enter valid numeric values for XMP pose settings")
            return
        ...
        self.log_calibration(
            "Calibration complete! XMP pose settings validated.")
        self.log_calibration(
            "You can now run capture sequences with XMP sidecars.")
    ...
    def reset_calibration(self):
        ...
        self.calibration_progress_text.delete(1.0, tk.END)
        self.log_calibration(
            "Calibration reset. Click 'Start Calibration' to begin again at 0°.")
    ...
    def save_calibration(self):
        ...
        if filename:
            try:
                ...
                messagebox.showinfo(
                    "Save", "Calibration data saved successfully!")
            except Exception as e:
        ...
        if filename:
            try:
                ...
                if "focus_positions" in loaded:
                    ...
                    self.calibration_data = self._normalize_calibration_data(
                        loaded["focus_positions"])
                    ...
                    if "xmp_settings" in loaded:
                        ...
                        if "lens_to_object_mm" in xmp_settings:
                            self.lens_to_object_mm.set(
                                float(xmp_settings["lens_to_object_mm"]))
                        if "rail_to_horizon_deg" in xmp_settings:
                            self.rail_to_horizon_deg.set(
                                float(xmp_settings["rail_to_horizon_deg"]))
                        if "focal_length_35mm" in xmp_settings:
                            self.focal_length_35mm.set(
                                float(xmp_settings["focal_length_35mm"]))
                        if "xmp_position_scale" in xmp_settings:
                            self.xmp_position_scale.set(
                                float(xmp_settings["xmp_position_scale"]))
                        # Intrinsics
                        if "principal_point_u" in xmp_settings:
                            self.principal_point_u.set(
                                float(xmp_settings["principal_point_u"]))
                        if "principal_point_v" in xmp_settings:
                            self.principal_point_v.set(
                                float(xmp_settings["principal_point_v"]))
                        if "aspect_ratio" in xmp_settings:
                            self.aspect_ratio.set(
                                float(xmp_settings["aspect_ratio"]))
                        if "skew" in xmp_settings:
                            self.skew.set(float(xmp_settings["skew"]))
                        # Distortion
                        if "distortion_model" in xmp_settings:
                            self.distortion_model.set(
                                str(xmp_settings["distortion_model"]))
                        if "distortion_k1" in xmp_settings:
                            self.distortion_k1.set(
                                float(xmp_settings["distortion_k1"]))
                        if "distortion_k2" in xmp_settings:
                            self.distortion_k2.set(
                                float(xmp_settings["distortion_k2"]))
                        if "distortion_k3" in xmp_settings:
                            self.distortion_k3.set(
                                float(xmp_settings["distortion_k3"]))
                        if "distortion_p1" in xmp_settings:
                            self.distortion_p1.set(
                                float(xmp_settings["distortion_p1"]))
                        if "distortion_p2" in xmp_settings:
                            self.distortion_p2.set(
                                float(xmp_settings["distortion_p2"]))
                        if "distortion_k4" in xmp_settings:
                            self.distortion_k4.set(
                                float(xmp_settings["distortion_k4"]))
                else:
                ...
                if self.validate_calibration_data():
                    ...
                    messagebox.showinfo(
                        "Load", "Calibration data loaded successfully!")
    ...
    def log_calibration(self, message):
        ...
        self.calibration_progress_text.insert(
        ...
        if abs(rotation_needed) > 0.01:
            ...
            if self.send_motor_command(command, wait_for_done=True):
                ...
                self.after(0, self.status_var.set,
                           f"Failed to move to {target_angle}°")
                return False
        return True
    ...
    def _build_guided_capture_tab(self):
        ...
        params_frame = ttk.LabelFrame(
            left_column, text="Scan Parameters", padding=10)
        params_frame.pack(fill="x", pady=(0, 5))

        # Stack depth
        ttk.Label(params_frame, text="Stack Depth (mm):").grid(
            row=0, column=0, sticky="w", pady=2)
        ...
        # Slices per stack
        ttk.Label(params_frame, text="Slices per Stack:").grid(
            row=1, column=0, sticky="w", pady=2)
        ...
        ttk.Label(params_frame, textvariable=self.guided_slice_step_var).grid(
            row=1, column=2, sticky="w", padx=10)

        # Stacks per rotation
        ttk.Label(params_frame, text="Stacks per Rotation:").grid(
            row=2, column=0, sticky="w", pady=2)
        ...
        # Near plane position
        ttk.Label(params_frame, text="Near Plane (mm):").grid(
            row=5, column=0, sticky="w", pady=2)
        ...
        list_frame = ttk.LabelFrame(
            left_column, text="Capture Plan", padding=10)
        ...
        self.guided_position_listbox.pack(
            side="left", fill="both", expand=True)
        ...
        ttk.Label(list_frame, textvariable=self.guided_summary_var).pack(
            pady=(5, 0))
        ...
        pos_frame = ttk.LabelFrame(
            right_column, text="Current Stack", padding=10)
        ...
        control_frame = ttk.LabelFrame(
            right_column, text="Controls", padding=10)
        ...
        self.guided_log_text = tk.Text(
            log_frame, height=6, width=40, font=("TkDefaultFont", 8))
    ...
    def _generate_guided_plan(self):
        ...
        self.guided_summary_var.set(
            f"{total} stacks × {self.guided_slices_per_stack.get()} slices = {total_images} images")
    ...
    def _start_guided_session(self):
        ...
        if not self.guided_position_list:
            messagebox.showwarning(
                "No Plan", "Please generate a capture plan first.")
            return

        # Create session directory
        specimen_dir = os.path.join(
            self.output_dir.get(), self.specimen_name.get())
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.guided_session_dir = os.path.join(
            specimen_dir, f"guided_session_{timestamp}")
    ...
    def _update_guided_display(self):
        ...
        if idx < total:
            ...
            self.guided_current_pos_var.set(
                f"Rot: {rot:.1f}°  Tilt: {tilt:+.1f}°  Rail: {rail:.1f}mm")
    ...
    def _guided_goto_position(self):
        ...
        self._log_guided(
            f"Moving to: Rot={rot:.1f}° Tilt={tilt:+.1f}° Rail={rail:.1f}mm")
        ...
        def move_sequence():
            ...
            self.after(0, lambda: self._log_guided(
                "Position reached - adjust specimen if needed"))
    ...
    def _guided_capture_stack(self):
        ...
        stack_dir = os.path.join(
            self.guided_session_dir, f"stack_{idx:0{pad_width}d}")
        os.makedirs(stack_dir, exist_ok=True)

        self._log_guided(
            f"Capturing stack {idx}: {slices} slices, {depth:.2f}mm depth")

        def capture_sequence():
            try:
                # Disable buttons during capture
                self.after(0, lambda: self.btn_guided_capture.config(
                    state="disabled"))
                self.after(0, lambda: self.btn_guided_goto.config(
                    state="disabled"))
                self.after(0, lambda: self.btn_guided_next.config(
                    state="disabled"))
                ...
                for slice_idx in range(slices):
                    ...
                    self.after(0, lambda s=slice_idx: self._log_guided(
                        f"  Slice {s+1}/{slices} captured"))

                # Generate XMP for this stack using current actual position
                self._generate_guided_xmp(idx, rot, tilt, stack_dir, pad_width)

                self.after(0, lambda: self._log_guided(
                    f"Stack {idx} complete"))
                self.after(0, lambda: self.status_var.set(
                    f"Stack {idx} captured successfully"))
                ...
                # Re-enable buttons
                self.after(
                    0, lambda: self.btn_guided_capture.config(state="normal"))
                self.after(
                    0, lambda: self.btn_guided_goto.config(state="normal"))
                self.after(
                    0, lambda: self.btn_guided_next.config(state="normal"))
    ...
    def _guided_next_position(self):
        ...
        if self.guided_current_index >= len(self.guided_position_list):
            ...
            self._update_guided_display()
            self._log_guided(
                f"Moved to position {self.guided_current_index + 1}")
    ...
    def move_to_elevation(self, target_elevation):
        ...
        if abs(tilt_needed) > 0.01:
            ...
            if self.send_motor_command(command, wait_for_done=True):
                ...
                self.after(0, self.motor3_pos_var.set,
                           f"{target_elevation:.1f}°")
                self.after(0, self.status_var.set,
                           f"Tilted to {target_elevation}°")
                return True
            else:
                self.after(0, self.status_var.set,
                           f"Failed to tilt to {target_elevation}°")
                return False
        return True
    ...
    def _update_scan_mode_ui(self):
        ...
        if self.scan_mode.get() == "spherical":
            ...
            self.spherical_settings_label.grid(row=5, column=0, sticky="w")
            self.spherical_min_label.grid(
                row=5, column=1, sticky="w", padx=(5, 2))
            self.spherical_min_spinbox.grid(
                row=5, column=1, sticky="w", padx=(30, 0))
            self.spherical_max_label.grid(
                row=5, column=1, sticky="w", padx=(100, 0))
            self.spherical_max_spinbox.grid(
                row=5, column=1, sticky="w", padx=(135, 0))
            self.spherical_steps_label.grid(
                row=5, column=2, sticky="w", padx=(10, 2))
            self.spherical_steps_spinbox.grid(
                row=5, column=2, sticky="w", padx=(55, 0))
        else:
    ...
    def apply_capture_preset(self):
        """Apply capture parameters from selected object size and quality preset."""
        size = (self.capture_preset_size.get() or "").strip().lower()
        quality = (self.capture_preset_quality.get() or "").strip().lower()

        preset = CAPTURE_PRESETS.get(size, {}).get(quality)
        if not preset:
            self.status_var.set(f"Invalid preset selection: {size}/{quality}")
            return

        self.stacks_count.set(int(preset["perspectives"]))
        self.shots_per_stack.set(int(preset["focus_slices"]))
        self.settle_delay.set(float(preset["settle_delay"]))

        self.capture_preset_applied_var.set(
            f"Preset applied: {size.title()} / {quality.title()} (editable)"
        )
        self.status_var.set(
            f"Applied preset {size}/{quality}: {preset['perspectives']} angles, "
            f"{preset['focus_slices']} slices"
        )
    ...
    def interpolate_focus_position(self, angle, stack_position):
        ...
        if angle in self.calibration_data:
            ...
            if lower_angle == upper_angle:
                ...
                angle_factor = (angle - lower_angle) / \
                ...
                near_pos = near_lower + \
                    (near_upper - near_lower) * angle_factor
    ...
    def start_full_capture(self):
        ...
        self.capture_thread = threading.Thread(
            target=self._run_capture_sequence)
    ...
    def _run_capture_sequence(self):
        ...
        try:
            ...
            self.after(0, lambda: self.btn_stop_capture.config(
                state="disabled"))
    ...
    def _run_ring_capture(self, session_dir, scan_mode):
        ...
        self.after(0, lambda: self.log_capture(
            f"Starting ring scan: {total_images} images"))
        ...
        for perspective in range(self.stacks_count.get()):
            ...
            if not self.move_to_angle(angle):
                self.after(0, self.log_capture,
                           f"Aborting: Failed to move to angle {angle}")
                break
            ...
            for slice_idx in range(self.shots_per_stack.get()):
                ...
                stack_dir = os.path.join(
                    session_dir, f"stack_{perspective:0{stack_pad_width}d}")
                os.makedirs(stack_dir, exist_ok=True)

                # Map slice index to 0.0..1.0 focus position
                stack_position = (slice_idx / (self.shots_per_stack.get() - 1)
                                  ) if self.shots_per_stack.get() > 1 else 0.0
                focus_position = self.interpolate_focus_position(
                    angle, stack_position)

                # Move focus
                if not self.move_to_focus_position(focus_position):
                    self.after(
                        0, self.log_capture, f"Aborting: Failed to move focus to {focus_position:.2f}mm")
                    break
                ...
                self.after(0, self.progress_label_var.set,
                           f"Captured {image_count}/{total_images} images")
                if image_count % 10 == 0:
                    self.after(0, self.log_capture,
                               f"Captured {image_count} images")

            # Generate XMP sidecar for this stack (perspective)
            self._generate_ring_xmp(
                session_dir, perspective, angle, stack_pad_width)

        # Consolidate XMP files
        self._consolidate_xmp_files(
            session_dir, image_count, angle_step, stack_pad_width)
    ...
    def _generate_ring_xmp(self, session_dir, perspective, angle, stack_pad_width):
        ...
        try:
            ...
            self.after(0, self.log_capture,
                       f"Generated XMP for stack {perspective} at {angle:.2f}°")

        except Exception as e:
            self.after(0, self.log_capture,
                       f"Warning: Failed to generate XMP for stack {perspective}: {e}")
    ...
    def _generate_spherical_xmp(self, session_dir, perspective_idx, azimuth, elevation, stack_pad_width):
        ...
        try:
            ...
            # Create XMP file for the stack
            stack_dir = os.path.join(
                session_dir, f"stack_{perspective_idx:0{stack_pad_width}d}")
            ...
            self.after(0, self.log_capture,
                       f"Generated XMP for stack {perspective_idx} (elev={elevation:.1f}°, azim={azimuth:.1f}°)")

        except Exception as e:
            self.after(0, self.log_capture,
                       f"Warning: Failed to generate XMP for stack {perspective_idx}: {e}")
    ...
    def _consolidate_xmp_files(self, session_dir, image_count, angle_step, stack_pad_width):
        ...
        try:
            ...
            helper_script_source = os.path.join(
                script_dir, "templates", "rename_xmp_for_rc.py")
            if os.path.exists(helper_script_source):
                helper_script_dest = os.path.join(
                    session_dir, "rename_xmp_for_rc.py")
                shutil.copy2(helper_script_source, helper_script_dest)
                self.after(0, lambda: self.log_capture(
                    "Helper script copied to session directory"))

            # Copy README guide
            readme_source = os.path.join(
                script_dir, "templates", "SESSION_README.md")
            if os.path.exists(readme_source):
                readme_dest = os.path.join(session_dir, "README.md")
                shutil.copy2(readme_source, readme_dest)
                self.after(0, lambda: self.log_capture(
                    "Workflow README copied to session directory"))

            self.after(0, lambda: self.log_capture(
                f"Consolidated {xmp_count} XMP files to: {xmp_dir}"))

        except Exception as e:
            self.after(0, lambda: self.log_capture(
                f"Warning: Failed to consolidate XMP files: {e}"))

        self.after(0, lambda: self.log_capture(
            f"Capture sequence complete! {image_count} images saved to {session_dir}"))
        self.after(0, lambda: messagebox.showinfo(
            "Complete", f"Capture sequence complete!\n{image_count} images saved.\nXMP files consolidated in xmp_files directory."))
    ...
    def _run_spherical_capture(self, session_dir, scan_mode):
        ...
        if elevation_steps_count == 1:
            ...
            elevation_step = (elevation_max - elevation_min) / \
                (elevation_steps_count - 1)
            elevations = [elevation_min + i *
                          elevation_step for i in range(elevation_steps_count)]
        ...
        self.after(0, lambda: self.log_capture(
            f"Starting spherical scan: {total_images} images"))
        ...
        if not self.move_to_elevation(elevations[0]):
            raise RuntimeError(
                f"Failed to move to starting elevation {elevations[0]}°")
        ...
        for elev_idx, elevation in enumerate(elevations):
            ...
            if not self.move_to_elevation(elevation):
                self.after(0, lambda e=elevation: self.log_capture(
                    f"Aborting: Failed to tilt to {e}°"))
                break

            self.after(0, lambda e=elevation, i=elev_idx: self.log_capture(
                f"Elevation {i+1}/{len(elevations)}: {e:.1f}°"))
            ...
            for azimuth_idx in range(azimuth_steps_count):
                ...
                if not self.move_to_angle(azimuth):
                    self.after(0, lambda a=azimuth: self.log_capture(
                        f"Aborting: Failed to move to azimuth {a}°"))
                    break
                ...
                for slice_idx in range(focus_slices):
                    ...
                    # Map slice index to 0.0..1.0 focus position
                    stack_position = (
                        slice_idx / (focus_slices - 1)) if focus_slices > 1 else 0.0
                    focus_position = self.interpolate_focus_position(
                        azimuth, stack_position)

                    # Move focus
                    if not self.move_to_focus_position(focus_position):
                        self.after(0, lambda fp=focus_position: self.log_capture(
                            f"Aborting: Failed to move focus to {fp:.2f}mm"))
                        break
                    ...
                    self.after(0, self.progress_label_var.set,
                               f"Captured {image_count}/{total_images} images")
                    if image_count % 10 == 0:
                        self.after(0, lambda ic=image_count: self.log_capture(
                            f"Captured {ic} images"))

                # Generate XMP for this perspective
                self._generate_spherical_xmp(
                    session_dir, perspective_idx, azimuth, elevation, stack_pad_width)
                perspective_idx += 1

        # Consolidate XMP files - use azimuth_step for consistency
        self._consolidate_xmp_files(
            session_dir, image_count, azimuth_step, stack_pad_width)
    ...
    def move_to_focus_position(self, target_position):
        ...
        if abs(move_distance) > 0.01:  # Only move if significant difference
            ...
            if self.send_motor_command(command, wait_for_done=True):
                ...
                self.after(0, self.motor2_pos_var.set,
                           f"{target_position:.2f}mm")
                return True
            else:
                self.after(0, self.status_var.set,
                           "Failed to move focus position")
                return False
        return True
    ...
    def update_camera_info(self):
        ...
        try:
            ...
            self.camera_info_text.config(state="normal")
            self.camera_info_text.delete(1.0, tk.END)
            self.camera_info_text.insert(
                1.0, f"Error getting camera info: {e}")
            self.camera_info_text.config(state="disabled")
...
https://github.com/reprahkcin/scanner-companion"""
    ...
    def _load_notes(self):
        ...
        notes_file = os.path.join(
            os.path.dirname(__file__), "session_notes.txt")
        try:
