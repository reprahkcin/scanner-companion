...
from camera import BaseCamera, PiCameraHQ, MockCamera, CameraSettings
...
# XMP pose calculation (extracted module)
...
# XMP pose functions and constants are now in xmp_pose.py (imported above)
...
class ScannerGUI(tk.Tk):
    ...
    def __init__(self):
        ...
        # === Camera (abstraction layer) ===
        self.cam: BaseCamera | None = None  # camera abstraction instance
        # Legacy alias – some internal helpers still reference self.camera
        # raw Picamera2 handle (set after connect for picamera2-specific calls)
        self.camera = None
  ...
  4. Enter k1-k4 and p1-p2 values here"""
        ...
        try:
            if self.cam is None:
                if PICAMERA2_OK:
                    ...
                    self.cam = MockCamera()
                self.cam.connect()
                # Keep legacy alias for any code that still touches self.camera
                self.camera = getattr(self.cam, '_cam', None)
            try:
                w, h = map(
                    int, self.preview_resolution.get().lower().split("x"))
            except Exception:
                w, h = CAMERA_RESOLUTION
            self.cam.start_stream(resolution=(w, h))
    ...
    def _update_manual_status(self):
        ...
        try:
            ...
            elif hasattr(self, 'cam') and self.cam is not None and self.cam.is_connected:
...
# =====================================================================
    ...
    def _update_scan_mode_ui(self):
        ...
        if self.scan_mode.get() == "spherical":
            ...
            self.spherical_min_label.grid(
                row=3, column=1, sticky="w", padx=(5, 2))
            self.spherical_min_spinbox.grid(
                row=3, column=1, sticky="w", padx=(30, 0))
            self.spherical_max_label.grid(
                row=3, column=1, sticky="w", padx=(100, 0))
            self.spherical_max_spinbox.grid(
                row=3, column=1, sticky="w", padx=(135, 0))
            self.spherical_steps_label.grid(
                row=3, column=2, sticky="w", padx=(10, 2))
            self.spherical_steps_spinbox.grid(
                row=3, column=2, sticky="w", padx=(55, 0))
        else:
    ...
    def capture_image(self, filepath):
        ...
        self.cam.capture_image(filepath, resolution=(w, h), fmt=fmt)
    ...
    def apply_camera_settings(self):
        ...
        try:
            # Determine analogue gain from ISO or direct gain setting
            gain = None
            if not self.ae_enable.get():
                iso = self.iso_value.get()
                if iso and iso != "Auto":
                    try:
                        gain = float(int(iso) / 100.0)
                    except Exception:
                        pass
            ...
            settings = CameraSettings(
                shutter_speed_us=self.shutter_speed.get() if self.shutter_speed.get() > 0 else None,
                ae_enable=self.ae_enable.get(),
                analogue_gain=gain,
                ev_compensation=self.ev_compensation.get() if self.ae_enable.get() else None,
                brightness=self.brightness.get(),
                awb_enable=self.awb_enable.get(),
                wb_red_gain=self.wb_red_gain.get(),
                wb_blue_gain=self.wb_blue_gain.get(),
                contrast=self.contrast.get(),
                saturation=self.saturation.get(),
                sharpness=self.sharpness.get(),
            )
            self.cam.apply_settings(settings)
